...
export async function getEmbedsFromDatabase(): Promise<Array<EmbedStructureWithId>> {
    ...
    if (error) {
        await useLogger("error", "Failed to get embeds from database", error.message);
        throw new Error(error.message);
    }

    const embeds = data.map((embed) => {
...
export async function useEmbedHandler(_dir: string): Promise<void> {
    //
}
